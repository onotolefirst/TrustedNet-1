...
-(BOOL) addFileToZip:(NSString*) file newname:(NSString*) newname;
{
	...
	NSError *someError;
	NSDictionary* attr = [[NSFileManager defaultManager] attributesOfItemAtPath:file error:&someError];
	if( attr )
	{
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite filePaths:(NSMutableArray *)arrPaths
{
	BOOL success = YES;
	...
	do
    {
        ...
        {
            if ( (unsigned char)filename[j] > '\x7f' )
            {
                bIsASCII = NO;
                break;
            }
        ...
        if (!bIsASCII)
        {
            int iSize = (fileInfo.size_filename + 2) * 2;
            char *utfFilename = (char*) malloc( iSize );
            utfFilename[0] = '\0';
            char *utfFilename2 = utfFilename;
            
            // convert it to utf8
            iconv_t ctxIconv = iconv_open( "UTF-8", "CP866");
            size_t sz_inbytes_left = fileInfo.size_filename;
            size_t sz_outbytes_left = iSize;
            size_t rez = 0;
        
            if (ctxIconv != (iconv_t)-1)
            {
                rez = iconv(ctxIconv, &filename, &sz_inbytes_left, &utfFilename, &sz_outbytes_left);
            }
            else
            {
                // TODO: throw error
            ...
            utfFilename2[iSize - sz_outbytes_left] = '\0'; // very important (!)
            
            // check if it contains directory
            strPath = [[NSString alloc] initWithCString:utfFilename2 encoding:NSUTF8StringEncoding];
            
            free ( utfFilename2 );
            iconv_close(ctxIconv);
        }
        else 
        {
            strPath = [[NSString alloc] initWithCString:filename encoding:NSASCIIStringEncoding];
            strPath = [strPath substringToIndex:fileInfo.size_filename];
            
            free( filename );
        }
        
		BOOL isDirectory = NO;
        if ( [strPath characterAtIndex:([strPath length] - 1)] == '/' || [strPath characterAtIndex:([strPath length] - 1)] == '\\')
        {
			isDirectory = YES;
            [arrPaths addObject:strPath];
        }
        
        if ((nil == strPath) || (![strPath length]))
        {
            unzCloseCurrentFile( _unzFile );
